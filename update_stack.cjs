...
async function run() {
    console.error('Fetching simple-icons data via fetch...');
    let icons;
    try {
    ...
    console.error('Fetched ' + icons.length + ' icons.');
    ...
    if (missing.length > 0) {
        console.error('Missing icons for ' + missing.length + ' items: ' + missing.slice(0, 5).join(', ') + '...');
    }
